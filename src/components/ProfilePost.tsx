  ...
  return (
    <div className="rounded-md overflow-hidden aspect-square group cursor-pointer relative">
      ...
      {isPostSafe ? (
        ...
        <div className="rounded-md absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-2 bg-[#fee0e0] border border-status-danger px-1 py-0.5">
          <AlertTriangle className="w-2 h-2 stroke-[#841313]" />
        </div>
      )}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-2 text-verySmall flex flex-col items-center justify-center">
        ...
        {isPostSafe ? (
          <div className="bg-[#f4fff1] border border-status-success px-2 py-1 text-[#13843c] rounded-xl">
            Genuine {trustScore.toFixed(0)}%
          </div>
        ) : (
          <div className="bg-[#fee0e0] border border-status-danger px-2 py-1 text-[#841313] rounded-xl">
            AI Content
          </div>
        )}
      </div>
